...
class DetectionState:
    def __init__(self):
        ...
        # Optimization: Track datetime directly to avoid strptime parsing in hot loop
...
async def detection_loop():
    ...
    while state.is_running:
        ...
        if state.fall_probability > state.threshold:
            # Optimization: Compare datetime objects instead of strptime parsing every frame
            now = datetime.now()
            if state.last_alert_time is None or (now - state.last_alert_time).total_seconds() >= 2.0:
                alert = {
