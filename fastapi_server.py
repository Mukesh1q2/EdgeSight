...
async def generate_video_frames():
    ...
    while state.is_running:
        ...
        # ⚡ Bolt: Offload CPU-bound image encoding to threadpool to prevent event loop blocking
        ret, buffer = await asyncio.to_thread(cv2.imencode, '.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 85])
        if ret:
            yield (b'--frame\r\n'
                   b'Content-Type: image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n')

        # ⚡ Bolt: Use async sleep to prevent thread blocking during stream interval
