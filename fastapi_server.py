...
class DetectionState:
    def __init__(self):
        ...
        self.last_alert_time = 0.0  # Float timestamp for fast rate-limiting
...
async def detection_loop():
    ...
    while state.is_running:
        ...
        if state.fall_probability > state.threshold:
            # ⚡ Bolt Optimization: Use float timestamp for rate-limiting
            # instead of expensive strptime() parsing (~200x faster).
            current_t = time.time()
