...
class DetectionState:
    def __init__(self):
        ...
        self.last_alert_time = 0.0  # Added for optimized alert cooldown tracking
...
async def detection_loop():
    ...
    while state.is_running:
        ...
        if state.fall_probability > state.threshold:
            # Optimize: use time.time() for the cooldown check instead of strptime
            current_time_sec = time.time()
            if not state.alerts or (current_time_sec - state.last_alert_time) >= 2.0:
                alert = {
                    "timestamp": datetime.now().strftime("%H:%M:%S"),
                    "probability": round(state.fall_probability, 3)
                }
                state.alerts.append(alert)
                state.last_alert_time = current_time_sec
