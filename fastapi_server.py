...
class DetectionState:
    def __init__(self):
        ...
        self.last_alert_time: Optional[datetime] = None  # Track alert time for optimized throttling
        self.frame_count = 0
...
async def detection_loop():
    ...
    while state.is_running:
        ...
        if state.fall_probability > state.threshold:
            now = datetime.now()
            # ⚡ Bolt Optimization: Use native datetime comparison instead of computationally expensive string parsing
            # datetime.strptime was slowing down the hot detection loop.
            if state.last_alert_time is None or (now - state.last_alert_time).seconds >= 2:
                alert = {
