...
async def detection_loop():
    ...
    while state.is_running:
        ...
        if state.fall_probability > state.threshold:
            ...
            # ⚡ Bolt: Defer expensive string formatting (strftime) and parsing (strptime)
            # until after the native rate limit check passes to save CPU cycles in the hot loop.
            current_time_for_alert = time.time()
            if not state.alerts or (current_time_for_alert - state.last_alert_time) >= 2.0:
                alert = {
                    "timestamp": datetime.now().strftime("%H:%M:%S"),
                    "probability": round(state.fall_probability, 3)
                }
                state.alerts.append(alert)
                state.last_alert_time = current_time_for_alert
